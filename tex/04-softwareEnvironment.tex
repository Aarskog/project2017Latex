...
As said above, the complete documentation on the different parts is not complete. This means that some of the lengths has to be guesstimated\cite{Crustcrawler}. In the simplified model (\figref{fig:naming}) there are only boxes except the turntable which is a flat cylinder. The inertia matrix \eqref{eq:inmat} is used:
